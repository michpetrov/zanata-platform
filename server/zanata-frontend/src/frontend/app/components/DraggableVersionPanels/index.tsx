...
export const tooltipSort = (<Tooltip id='tooltipsort'>Best match will be chosen based on the priority of
    selected projects. Exact matches take precendence.
  </Tooltip>);
export const DragHandle = SortableHandle(() => <Icon name='menu' className='n1 drag-handle' title='click to drag'/>);
type ItemProps = {
    value: any;
...
export class Item extends Component<ItemProps, ItemState> {
    ...
    render() {
      ...
      {" "}
      <Button bsSize='xsmall' className='close rm-version-btn' onClick={this.removeVersion}>
...
class Items extends Component<{
    ...
    render() {
        ...
        const sortableItems = items.map((value, index) => (<SortableItem key={value.projectSlug + ':' + value.version.id} index={index} value={value} removeVersion={removeVersion}/>));
        return (<div>
...
class DraggableVersionPanels extends Component<{
    ...
    render() {
        if (this.props.selectedVersions.length === 0) {
            return (<span className="no-v text-muted">
          Please select versions to sort<br />
          <Icon name="version" className="s8"/>
        </span>);
        }
        return (<ListGroup>
        <SortableList items={this.props.selectedVersions} onSortEnd={this.props.onDraggableMoveEnd} useDragHandle removeVersion={this.props.removeVersion} helperClass='sortable-helper'/>
      </ListGroup>);
    }
}
